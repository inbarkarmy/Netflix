...
j = 0
FILE_NAME = 'database_new.csv'

TITLES_LIST = ['id','title', 'year', 'Time', 'country', 'director',
...
#print("got to start")
file = open('FixDatabase.csv')
next(file)
csvlLine1 = [0]*len(TITLES_LIST)
for line in file:
    old_line = line
    id, name, year, rest = line.split(',', 3)
    #print(year)
    if any(c.isalpha() for c in year) and year is not "year":
        #print("got to if")
        #print(line)
        tmp = line
        k = 0
        id = line[:line.find(',')]
        maybeyear = line.split(',', 13)
        for a in maybeyear:
            if not any(c.isalpha() for c in a) and (k != 0 and k != 1) :
                year = a
                break
            k = k + 1
        #print(year)
        nameforserch = line[line.find(',') + 1: line.find(year) - 1]
        #print(nameforserch)
        name = nameforserch.replace(',', '')
        #print(name)
